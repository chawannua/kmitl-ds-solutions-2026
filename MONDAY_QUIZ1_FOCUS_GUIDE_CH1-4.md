...
### 💡 Stack Application 1: Parentheses Matching
...
### 💡 Stack Application 3: Postfix Expression Evaluation
...
### 🏢 1. Parking Lot Simulation (Stack)
...
### ☕ 2. Multi-Barista / Multi-Cashier Queue Simulation
...
1. **Popping from an Empty Stack/Queue**:
   - ⚠️ *Mistake*: Calling `stack.pop()` when `len(stack) == 0` causes `IndexError: pop from empty list`.
   - ✅ *Fix*: Always check `if stack:` or `if not stack.is_empty():` before popping.

2. **Infix to Postfix Operator Precedence**:
   - ⚠️ *Mistake*: Forgetting that `(` has precedence inside the stack until `)` is encountered.
   - ✅ *Fix*: When encountering `(`, push to stack. When encountering `)`, pop everything until `(` is popped.

3. **String Modulo & Floating Point Precision**:
   - ⚠️ *Mistake*: Outputting `3000.0` instead of `3000.00`.
   - ✅ *Fix*: Use `f"{value:.2f}"`.

4. **Python `pop(0)` Efficiency**:
   - ⚠️ *Mistake*: Using `list.pop(0)` in large loops can cause Time Limit Exceeded (TLE) because shifting items is $O(N)$.
   - ✅ *Fix*: Use `from collections import deque` and `deque.popleft()` for $O(1)$ queue operations.

5. **Modifying List While Iterating**:
   - ⚠️ *Mistake*: `for item in queue: queue.remove(item)` skips elements!
   - ✅ *Fix*: Iterate over a copy (`for item in list(queue):`) or use `while queue:`.

---

## 🎯 Quick Self-Test Practice Questions
...
---
*Good luck on Monday's Quiz! You've got this! 💯*
